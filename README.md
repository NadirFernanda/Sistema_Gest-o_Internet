...
## ⚡ Comandos rápidos do servidor (SSH)

### Carregar saldo WhatsApp (no servidor via SSH)

```bash
cd /var/www/sgmr
php artisan tinker --execute="App\Models\WhatsappLedger::carregar(20000, 'Carregamento manual', 1);"
```

> Substitui `20000` pelo valor em Kz e `1` pelo ID do teu utilizador admin.
> Para ver o saldo actual: `php artisan tinker --execute="echo App\Models\WhatsappLedger::saldoAtual();"`

### Actualizar o sistema (deploy)

```bash
cd /var/www/sgmr
git pull origin main && php artisan view:clear && systemctl restart php8.4-fpm
```
...
## Configuração de produção (exemplo resumido)
