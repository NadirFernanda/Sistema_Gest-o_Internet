...
class Pay4AllService
{
    ...
    public function getToken(): string
    {
        return Cache::remember('pay4all_access_token', 3300, function () {
            ...
            if ($response->failed()) {
                Log::error('Pay4All: falha ao obter token', [
     ...
     *
     * @param  float  $amount             Valor a cobrar (em AOA)
     * @param  string $phoneNumber        Número de telemóvel do cliente (com ou sem indicativo 244)
     * @param  string $transactionId      ID único da transação (max 15 chars, [a-zA-Z0-9])
     * @param  string $description        Descrição curta (sem caracteres especiais)
     * @param  string|null $customerName  Nome do cliente (para notificação)
    ...
    public function createCharge(
        ...
        string $description = 'Pagamento SGA',
        ...
        $response = Http::withToken($token)
            ->withHeaders([
                ...
                'Accept'          => 'application/vnd.appypay.asyncapi+json', // modo assíncrono
                'Accept-Language' => 'pt-BR',
            ])
            ->post("{$this->apiUrl}/charges", $payload);

        Log::info('Pay4All: createCharge', [
        ...
        if ($response->failed() && $response->status() !== 202) {
            Log::error('Pay4All: erro ao criar cobrança', [
                'status' => $response->status(),
                'body'   => $response->body(),
            ]);
