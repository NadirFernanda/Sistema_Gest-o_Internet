...
<div class="container" style="max-width: 560px; margin: 40px auto;">
    ...
    <h2 style="text-align:center; margin-bottom:1.5rem;">Adicionar Produto ao Catálogo</h2>
    ...
    <form action="{{ route('catalog_equipamentos.store') }}" method="POST" class="form-modern">
        @csrf

        <div class="form-group">
            <label for="nome">Nome do produto <span class="text-danger" style="display:inline;">*</span></label>
            <input type="text" id="nome" name="nome" value="{{ old('nome') }}" placeholder="Ex: Router TP-Link TL-WR840N">
            @error('nome')<span class="text-danger">{{ $message }}</span>@enderror
        </div>

        <div class="form-group">
            <label for="categoria">Categoria</label>
            <input type="text" id="categoria" name="categoria" value="{{ old('categoria') }}" placeholder="Ex: Routers, Repetidores, Antenas, Cabos">
        ...
        <div class="form-group">
            ...
            <input type="number" id="preco" name="preco" value="{{ old('preco', 0) }}" min="0" step="1">
        ...
        <div class="form-group">
            ...
            <input type="number" id="quantidade" name="quantidade" value="{{ old('quantidade', 1) }}" min="0" step="1">
            @error('quantidade')<span class="text-danger">{{ $message }}</span>@enderror
        </div>

        <div class="form-group">
            <label for="descricao">Descrição</label>
            <textarea id="descricao" name="descricao" rows="3" placeholder="Descrição curta do produto...">{{ old('descricao') }}</textarea>
            @error('descricao')<span class="text-danger">{{ $message }}</span>@enderror
        </div>

        <div class="form-group">
            <label for="imagem_url">URL da imagem (opcional)</label>
            <input type="url" id="imagem_url" name="imagem_url" value="{{ old('imagem_url') }}" placeholder="https://...">
            @error('imagem_url')<span class="text-danger">{{ $message }}</span>@enderror
        </div>

        <div class="form-group">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                <input type="checkbox" name="ativo" value="1" style="width:auto;" {{ old('ativo', '1') == '1' ? 'checked' : '' }}>
                Visível na loja
            </label>
        </div>
