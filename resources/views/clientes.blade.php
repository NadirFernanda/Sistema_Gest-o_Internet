...
@section('content')
    <div class="clientes-container">
        ...
        @if(isset($clientes))
            ...
            <style>
                /* Match planos search: white rounded input with orange button on the right */
                .busca-planos-form { margin:12px 0 4px 0; display:flex; gap:12px; align-items:center; }
                .busca-planos-input {
                    flex:1; min-width:220px; height:48px; padding:12px 16px; border-radius:8px; border:1px solid #d9d9d9;
                    background:#fff; font-size:1rem; box-shadow:0 6px 18px rgba(0,0,0,0.04); color:#222;
                }
                .busca-planos-input::placeholder { color:#9b9b9b; }
                .busca-planos-btn {
                    height:48px; padding:0 18px; border-radius:8px; border:none; background:#e09b00; color:#fff; font-weight:700;
            ...
            <form method="GET" action="{{ url('/clientes') }}" id="formBuscaCliente" class="busca-planos-form">
                <input type="text" name="busca" id="buscaClientes" value="{{ request('busca') }}" placeholder="Pesquisar cliente por nome, BI, email ou contato" class="busca-planos-input" autocomplete="off">
